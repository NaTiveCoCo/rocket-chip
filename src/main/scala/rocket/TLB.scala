...
class TLB(instruction: Boolean, lgMaxSize: Int, cfg: TLBConfig)(implicit edge: TLEdgeOut, p: Parameters) extends CoreModule()(p) {
  ...
  /** AS may bootstrap in Bare; AU must have a protected paged root.
    * Keep this gate independent of vm_enabled, which is false in Bare.
    * Instruction requests use current privilege/world; data requests use the
    * effective privilege/world, including MPRV+MPP+MPA. Physical PTW requests
    * bypass this gate, while final-PA permissions and sticky fatal still apply.
    */
  val naccAUBareDenied = coreParams.hasNACC.B && naccAgentMode &&
    priv === PRV.U.U && !stage1_en && !io.req.bits.passthrough && !naccBitmapFatalApplies
